...
reply to every thread with `@coderabbitai`, then reconcile the current
pre-merge checks. A posted request, completed review, resolved thread,
approval, green CI, and successful integration are different facts. Never
...
configuration, not defaults supplied by this skill. Do not copy credentials
into comments or evidence records.
...
Each case separates the recorded incident, recovery, and evidence needed to
exit.

[Evidence and rehearsal](references/evidence-and-rehearsal.md) contains the
handoff and reconciliation templates, incident references, and offline
scenarios for checking this workflow. Historical counts and outcomes are
examples, not current PR status or estate-wide thresholds.
...
skill: `list`, `put`, `hist`, and `del`. Confirm the installed interface from
its source or an approved non-mutating probe. A different CLI is a deployment
issue, not permission to install or invoke a guessed replacement.
...
For remote invocation, preserve the comment body as one remote argument. Use
the configured host in place of the placeholder:
...
identity. Focused replies to existing findings or pre-merge rows are a
different operation: use only the project's already authorized reply route and
...
it is not a completed review or a promise of an unscheduled background check.
...
4. Read execution warnings as well as findings. A clone failure, missing
   required inspection, or inconclusive check cannot establish a clean review.
...
Repeat implementation and validation when a real defect remains. Request
another full review only when the remaining inspection need warrants one; a
corrected comment, title, percentage, or stale table alone does not
automatically require it. Preserve the actual scope of any earlier substantive
review.

## Delegate verification, repair, and validation

Use a wyvern agent team to verify every inline finding and pre-merge row
against the current code before editing. Keep this reconnaissance read-only:
inspect implementations, callers, contracts, tests, and documentation, and
record the current base/head, finding ID, evidence, and disposition. Ask the
scrutineer to execute any reproductions needed to decide validity. An old
comment anchor alone neither invalidates a finding nor proves that it still
needs work.
...
use the
[uncertain-comment template](references/evidence-and-rehearsal.md#uncertain-comment-disposition).
...
workers must not start competing gates. Its summary must identify the
candidate, commands, exit statuses, passed/failed/skipped results, log
locations, and any unexecuted or unavailable checks. Keep inspection claims
separate from execution evidence. After a repair or replay changes the
candidate, renew the relevant verification and validation before claiming
convergence.
...
Use a review submission's `commit_id` to identify that review's candidate. An
inline comment's `original_commit_id` identifies its original anchor, not proof
of what a later follow-up inspected. Recheck older findings against the new
tree rather than assuming that an old anchor makes them false.

## Dispositions and convergence

Treat both warning and error rows as requiring an explicit disposition;
warnings are not optional or aspirational. Mark a row resolved only with
evidence. Mark inherited findings with their comparison and owner; unchanged
test bytes alone do not prove unchanged runtime causality. Argue incorrect or
contradictory recommendations with source or tests. For genuinely out-of-scope
work, look for an existing GitHub issue and reference it, or propose one if
none exists. Do not silently dismiss the work or create duplicate issues.
Observability (o11y), code safety, documentation, and validation coverage for
changes made by this PR remain in scope, even when the implementation change is
small.

Use the
[pre-merge checks template](references/evidence-and-rehearsal.md#pre-merge-checks-reconciliation)
...
and submit its evidence. Do not claim that local evidence retroactively
completed a failed hosted inspection. Required service coverage still needs an
explicit accepted disposition through the existing review policy.
...
`APPROVED`; stale `CHANGES_REQUESTED` needs explicit reconciliation and
read-back.
...
base/head and treat previous merge eligibility as stale. Retain prior evidence
for its actual scope, run the required new-candidate gates, and establish the
inspection needed for the new delta. Never relabel an old full review as a new
one.
...
unknown review coverage, or a mismatched head triggers an andon: stop
advancing, preserve evidence, and return the unresolved boundary to its
designated owner.

## Installation boundary

`install-skills` copies immediate skill directories into the agent skill paths.
This directory therefore supplies the `comenq-coderabbit` name in this
repository. Before rollout, compare any independently installed copy with this
distribution and choose one authoritative version. Keep approved connection and
identity settings in deployment configuration; do not rely on installer
ordering to combine two different copies. This change does not modify another
skill repository, install the queue service, or authorize new account access.
