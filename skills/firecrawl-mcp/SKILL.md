...
operates on multiple URLs and uses Firecrawl's server-side LLM. The JSON format
on `firecrawl_scrape` works on a single page and also uses server-side
extraction. Prefer `firecrawl_extract` when pulling uniform structured data
from several pages. Prefer scrape with JSON format when you want markdown *and*
structured data from the same single page in one call.
...
**Source types** via `sources`: `["web"]` (default), `["news"]`, `["images"]`,
or combinations. The `limit` applies per source type.

**Category filtering** via `categories`: `["github"]`, `["research"]`,
`["pdf"]`. Narrows results to specific domains (GitHub repos, academic sites,
...
The `search` parameter filters returned URLs by relevance to a term — useful
when you only need the authentication docs from a large site, for instance.

Set `ignoreQueryParameters: true` to deduplicate URLs that differ only by query
string.
...
structure guided by `prompt`. Providing a schema is strongly recommended for
...
The agent is async — it returns a job ID. Poll `firecrawl_agent_status` every
15–30 seconds. Allow at least 2–3 minutes before treating it as failed. The
agent autonomously searches, navigates, and extracts.
...
**Always delete sessions when done.** Sessions have TTLs but leaving them open
wastes resources.
