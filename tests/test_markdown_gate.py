...
def test_ci_workflow_lints_through_the_pinned_action() -> None:
    ...
    the version Dependabot manages is the version that runs. The estate
    `markdown-formatting-baseline` rule (PD-006) requires a full commit SHA
    rather than a release tag: even an exact tag can be moved, which would
    change the gate under a passing pull request.
    """
    ...
    assert re.fullmatch(r"[0-9a-f]{40}", uses.group(1)), uses.group(1)
