...
@contextmanager
def _tmp_path() -> Iterator[Path]:
    """Create a temporary directory path for one Hypothesis example."""
    with tempfile.TemporaryDirectory(prefix="bootstrap-common-properties-") as tmpdir:
        yield Path(tmpdir)
...
def test_replace_managed_block_rejects_unbalanced_or_duplicate_sentinels(
    ...
    with _tmp_path() as tmp_path:
        ...
        contents = target.read_text()
        if (begin_count == end_count == 1) or (begin_count == end_count == 0):
            assert result.returncode == 0, result.stderr
            assert contents.count(begin_sentinel) == 1, contents
            assert contents.count(end_sentinel) == 1, contents
        else:
            assert result.returncode != 0, (
                ...
                f"begin_count={begin_count}; end_count={end_count}; "
...
def test_ensure_profile_path_is_idempotent_for_repeated_calls(call_count: int) -> None:
    ...
    with _tmp_path() as tmp_path:
        ...
        calls = "\n".join(
            f"ensure_profile_path {str(tool_dir)!r}" for _ in range(call_count)
        )
