...
`rstest-bdd` builds, tests, and lints on stable Rust. The library pins that
...
These blocks use `rust,ignore` because the doctest dependency upstream is a
vendored gpui shim. They reproduce the published `gpui 0.2.2` signatures;
...
full Whitaker suite includes `no_unwrap_or_else_panic`, so
`unwrap_or_else(|| panic!(…))` is rejected even when it encodes an
infrastructure invariant. The library also denies `clippy::expect_used` and
...
It is not enabled for these crates, and step functions and their helpers are
...
current lint gate; [ADR-013][adr-013] records the current compatibility
contract.
...
Under cargo-nextest (`cargo nextest run`, which the library's own test gate
uses), each test is run in its own process. The `#[serial]` mutex is not
